...
```{r}
library(scales)
ggplot(fb,aes(age_group,tenure))+geom_bar(stat='identity')+facet_wrap(~gender,nrow=2)+scale_y_continuous(labels = comma)
ggplot(fb,aes(age,log2(tenure)))+geom_point(alpha=0.5)+facet_wrap(~gender,nrow=2)

 
```

- From above we can infer that males are socially more active compared to females

- The distribution is right skewed, that means there are less number of crazy facebook users which is understandable
