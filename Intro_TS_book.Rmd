...
Z.ar$ar + c(-2, 2) * sqrt(Z.ar$asy.var.coef)
acf(Z.ar$res[-1])
pacf(Z.ar$res[-1])
...
pacf(Global.ar$res, na.action=na.pass)
...
ar1 <- acf(temp.lm$residuals,lag=50)
ar1$acf[2]

temp.gls <- gls(temp ~ time(temp), cor = corAR1(ar1$acf[2]))

summary(temp.gls)

temp.gls$coefficients
coef(temp.gls)
confint(temp.gls)
...
ar1 <- acf(temp.lm$residuals)
ar1$acf[2]

library(nlme)
temp.gls1 <- gls(temp ~ 0 + Time + factor(Seas), cor = corAR1(ar1$acf[2]))
...
acf(x.ma$res,na.action = na.pass)
...
for (p in 0:5)
{
  ...
  {
    #best_order <- c(p,0,q)
    temp <- AIC(arima(waveht, order = c(p,0,q)))
    if (temp < best_aic)
    {
      best_aic <- temp
      best_order <- c(p,0,q)
...
acf(wave.arma$residuals)

pacf(wave.arma$residuals)
hist(wave.arma$residuals, xlab='height / mm', main='')
...
#stemp <- read.table('stemp.dat', header = TRUE)
...
acf(stemp.arima$residuals)
acf((stemp.arima$residuals)^2)

stemp.garch <- garch(stemp.arima$residuals, trace = F)
confint(stemp.garch)

acf(stemp.garch$residuals,na.action = na.pass)
acf((stemp.garch$residuals)^2,na.action = na.pass)
