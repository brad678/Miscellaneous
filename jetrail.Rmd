...
model_xgb <- train(x_train[,predictors2],
                  x_train$Count,
                  method = 'xgbTree',
                  verbose = FALSE,
...
model_xgb1 <- train(x_train[,predictors],
                  x_train$Count,
                  method = 'xgbLinear',
                  verbose = FALSE,
                  trControl = fitcontrol)
...
#prediction
predict_model_xgb <- predict(model_xgb,x_test[,predictors2])


```

```{r}
#test_op <- data.frame(test$ID, predict_model_xgb)
...
write_csv(test_op, file.path(dir, "xgb4.csv"), col_names = TRUE)
...
for(i in 0:23)
{
  ...
  SMA_i <- sma(ts(temp_train$Count, frequency =365, start=c(2012,8,25)), h=nrow(temp_test), silent="graph")
  test_op <- data.frame(ID=temp_test$ID,Count=SMA_i$forecast)
  colnames(test_op) <- c("ID","Count")
  write_csv(test_op, file.path(dir, "sma.csv"), col_names=ifelse(i %in% 0, TRUE, FALSE), append=TRUE)
  print(i)
