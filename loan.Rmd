...
for (i in 1:k)
  ...
  mymodel <- knn(train=trainingset,test=testset,cl=trainingset[,"Loan_Status"],k=6)
  
  
  # predict Loan Status
  
  c1 <- confusionMatrix(table(predicted=mymodel,actual=testset$Loan_Status),positive = '2')
...
for (i in 1:k)
  ...
  mymodel <- knn(train=trainingset,test=testset,cl=trainingset[,"Loan_Status"],k=j)
