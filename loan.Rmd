...
logit.modelt <- glm(Loan_Status ~ Credit_History+Property_Area+LoanAmount+Married+log(ApplicantIncome), family = 'binomial', data = app_train)


summary(logit.modelt)

probs = predict(logit.modelt,app_val,type='response')

library(caret)

glm.pred <- rep('N',length(app_val$Loan_Status))
glm.pred[probs>0.5]='Y'
t1 <- table(predicted=glm.pred,actual=app_val$Loan_Status)
confusionMatrix(t1,positive = 'Y')
...
- It is fine not to approve the loan for genuine cases but it is risky to approve the loan for likely defaulter.So there is need to increase the True negative rate(specificity)


```{r}

glm.pred <- rep('N',length(app_val$Loan_Status))
glm.pred[probs>0.625]='Y'
t1 <- table(predicted=glm.pred,actual=app_val$Loan_Status)
confusionMatrix(t1,positive = 'Y')
