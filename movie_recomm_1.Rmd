...
model_xgb <- train(movies_train1[,predictors],
                  movies_train1[,outcome],
                  method = 'xgbTree',
                  verbose = FALSE,
                  trControl = fitcontrol)
...
model_xgb <- train(pca.train[,predictors],
                 pca.traib[,outcome],
                  method = 'xgbTree',
                  verbose = FALSE,
                  trControl = fitcontrol)
...
#prediction
predict_model_xgb <- predict(model_xgb,movies_test1[,predictors])

#calculating error measures
Error_measures(movies_test1$imdb_score,predict_model_xgb)
...
#GBM as top layer model (Gradient Boosting)
model_xgb_top <- train(movies_train1[,predictors_top],movies_train1[,outcome],method='xgbTree',trControl=fitcontrol,verbose = FALSE)
#verbose: logical. Should R report extra information on progress? If TRUE then report progress
model_xgb_top
...
```{r}
#predict using GBM top layer model
#movies_test1$gbm_stacked<-predict(model_xgb_top,movies_test1[,predictors_top])
xgbp<-predict(model_xgb_top,movies_test1[,predictors_top])
...
#calculating error measures
#Error_measures(movies_test1$imdb_score,movies_test1$gbm_stacked)
Error_measures(movies_test1$imdb_score,xgbp)
